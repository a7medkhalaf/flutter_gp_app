...
import 'package:flutter_gp_app/database/dummy_data/dummy_data.dart';
...
class DiaryEntryProvider with ChangeNotifier {
  ...
  Future<List<DiaryEntry>> get diaryEntries async {
    if (_diaryEntries.isEmpty) {
      _diaryEntries = await HiveProvider().getAllDiaries();
      if (_diaryEntries.isEmpty) {
  ...
  void addUpdateDiaryEntry({
    ...
    if (id != null) {
      ...
      HiveProvider().updateDiary(diaryEntry);
    } else {
      try {
        ...
        HiveProvider().insertDiary(diaryEntry);
      } catch (e) {
        log(e.toString());
      }
    }
  ...
  void removeDiaryEntry(diaryId) {
    ...
    HiveProvider().deleteDiary(diaryId);
    notifyListeners();
  }

  Future<void> resetDiaryEntries() async {
    _diaryEntries.clear();
    _diaryEntriesByMonth.clear();
    HiveProvider().deleteAllDiaries();
    await diaryEntries;
    notifyListeners();
  }
}
